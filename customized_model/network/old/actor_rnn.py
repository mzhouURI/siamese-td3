...
class VehicleActor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, rnn_layers=1, max_action=1):
        super().__init__()
        self.max_action = max_action
        self.fc_pre = nn.Linear(state_dim, hidden_dim)
        self.lstm = nn.LSTM(hidden_dim, hidden_dim, num_layers=rnn_layers, batch_first=True)
    ...
    def forward(self, state, seq_len):
        """
        state: [batch, state_dim]
        seq_len: int (number of future actions to generate)
        Returns:
        ...
        x = self.layernorm(state)
        x = torch.tanh(self.fc_pre(x))  # [batch, hidden_dim]

        # Expand x to a sequence: [batch, seq_len, hidden_dim]
        x_seq = x.unsqueeze(1).repeat(1, seq_len, 1)

        out, _ = self.lstm(x_seq)  # [batch, seq_len, hidden_dim]
        actions = self.max_action * torch.tanh(self.fc_out(out))  # [batch, seq_len, action_dim]
        return actions
