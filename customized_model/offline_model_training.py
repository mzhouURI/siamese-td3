...
# Remove pitch and yaw columns from `states`
new_base_states = data[1:, [15, 19, 23, 24]]
# Trim cos/sin arrays to match states shape (one less row due to data[:-1])
# Stack new states: [x, cos_pitch, sin_pitch, cos_yaw, sin_yaw, z, vx, vy]
new_states = np.column_stack((new_base_states[:, 0],   # col 15 (x)
                          cos_pitch,
                          sin_pitch,
                          cos_yaw,
                          sin_yaw,
                          new_base_states[:, 1:],  # cols 19, 23, 24 (z, vx, vy)
                         ))
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        # exit()
        loss = nn.HuberLoss(reduction="sum")(new_state_seq, pred_new_state_seq)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            #         pre_pitch = torch.atan2(flat_pred[:,2], flat_pred[:,1])
            #         actual_pitch = torch.atan2(flat_actual[:,2], flat_actual[:,1])
            #         ax1.plot(pre_pitch.detach().cpu().numpy(), label='Label (optional)', color='red', linestyle='None', marker='o')  # Customize as needed
            #         ax1.plot(actual_pitch.detach().cpu().numpy(), label='Label (optional)', color='blue', linestyle='None', marker='o')  # Customize as needed
                    
            #         pre_yaw = torch.atan2(flat_pred[:,4], flat_pred[:,3])
            #         actual_yaw = torch.atan2(flat_actual[:,4], flat_actual[:,3])
            #         ax2.plot(pre_yaw.detach().cpu().numpy(), label='Label (optional)', color='red', linestyle='None', marker='o')  # Customize as needed
            #         ax2.plot(actual_yaw.detach().cpu().numpy(), label='Label (optional)', color='blue', linestyle='None', marker='o')  # Customize as needed
            #         plt.show()
            total_val_loss += nn.HuberLoss(reduction="sum")(pred_new_state_seq, new_state_seq).item()
            batch_count += 1
    # Compute mean losses
